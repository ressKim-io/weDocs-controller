...
# 이 코퍼스 실측(2026-07-28): 변경 전/후 커밋에서 각각 실제 세션을 띄워
# 세션 시작 입력 토큰을 비교했다 — 61,615 → 28,070 (−33,545 토큰),
# 같은 구간의 메모리 자수 감소가 44,835자 ⇒ 1.34 자/토큰.
# (처음 추정한 1.99는 한글 토큰 밀도를 과소평가한 값이었다.)
# 사람이 감각을 잡기 위한 표시용 참고치일 뿐 — 게이트 판정은 항상 코드포인트로 한다.
CHARS_PER_TOKEN = 1.34
...
TIER_A = {
    ...
    # stage2: 내용 정리까지 끝난 시점의 실측 300줄 / 12,296자에 소폭 헤드룸.
    # 계획서 추정(300줄 / 9,000자)은 줄수는 맞췄지만 자수는 한글 밀도를 낮게 본 값이라
    # 그대로 쓰면 첫날부터 red. 여기서도 임계값은 실측을 따른다.
    # 헤드룸은 "정당한 소폭 추가는 통과, 드리프트 복귀는 차단"이 되도록 ~13%.
    "stage2": {
        "total_lines": 340,
        "total_chars": 14000,
        "claude_md_lines": 85,
