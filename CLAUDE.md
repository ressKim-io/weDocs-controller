...
- **다음 = Phase 2 2a-2 인가 🔄 착수(2026-07-20)** — PR #16 머지됨(`583b065`) → ws-gateway 새 브랜치: doc-service `CheckPermission` gRPC 클라이언트(blocking+deadline·fail-closed 3상태) + 핸드셰이크 authz(403)·viewer write-drop 1차·role 메타 엔진 전달 + `checkpermission_duration`/`authz_backend_error_total`(관측 계약 이어짐 — 2a-2가 `ws_handshake_total`에 authz_denied·backend_error 추가). ⚠️ **D1 = 비UUID `doc_id`/`user_id`는 gRPC 없이 403**(doc-service가 UUID 파싱 필수 → 기존 통합테스트 room·subject UUID화 필연) · **D2 = 단일 PR**(authz만 선머지 시 viewer 쓰기 구멍). 이후 2b(engine role 방어)→2c(frontend 토큰). M2 본류 후속 = Phase 3 엔진저장(build_client flip)·4 복원·5 outbox·6 E2E. **재개 상세 SSOT = `docs/plans/2026-07-19-m2-phase2-auth-authz.md` §재개 지점**. ⚠️ VT pinning 재검 = 2a-2(CheckPermission 블로킹 gRPC, JFR 실측) · ws-gateway 테스트 colima 불요 / doc-service만 필요.
