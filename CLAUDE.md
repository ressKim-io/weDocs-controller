...
- **M2 구현 진행 중 — Phase 1a ✅ ([backend PR #3](https://github.com/ressKim-io/weDocs-backend/pull/3) OPEN)**: doc-service 모듈 + Flyway page-tree 7테이블 + 핵심 엔티티 4종 + Testcontainers 영속 테스트 3건 green(브랜치 `feature/m2-doc-service-skeleton`). 빌드 함정 2건(Spring Boot 4.x autoconfig 모듈화→스타터 필수·TC 2.x 좌표) = `docs/dev-logs/2026-06-30-m2-doc-service-1a-version-traps.md`.
- **다음 = Phase 1b(gRPC `DocService` 4 RPC)** — PR #3 머지 후, 새 브랜치. 이어 1c REST/JWT → Phase 2 인증 / 3 엔진 저장(build_client flip) / 4 복원 / 5 outbox / 6 E2E. 횡단(T4) 병행.

> **재개 SSOT(M2 Phase 1b 진입)**: 진입점 = **`docs/plans/2026-06-30-m2-persistence-session.md`** §재개지점(1a=backend PR #3 OPEN, 다음=1b gRPC). Phase 1 분해(1a/1b/1c)·구현 패턴은 같은 plan 참조. 게이트 트랙 = `docs/plans/2026-06-30-plan-audit-improvements.md`(T3 done·T4). M2 ADR = `docs/adr/0012`~`0015`. M2 readiness 후향 = `docs/dev-logs/2026-06-30-m2-readiness-gate.md` · 1a 빌드함정 = `docs/dev-logs/2026-06-30-m2-doc-service-1a-version-traps.md`. M1 회고 = `docs/retrospective/2026-06-30-m1-convergence.md`. ⚠️ 서비스 레포(backend/crdt-engine/frontend)는 branch+PR+건별 승인·push 승인. controller만 main 직접.
