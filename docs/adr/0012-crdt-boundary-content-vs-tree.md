...
**왜 트리를 CRDT로 안 하나** (PRD §3.2): (a) 페이지 이동은 내용 편집보다 **압도적으로 드물고**, (b) 충돌 시 "마지막 이동 우선 + 사이클 방지"로 충분하며, (c) 트리 CRDT는 구현 난이도 대비 MLP 효용이 낮다.

## 결과

- M2 스키마가 두 저장소로 명확히 갈린다: `page_snapshots`(CRDT blob, bytea) vs `pages`/`page_permissions`(관계형 트리·권한). SDD §5 갱신의 근거.
- 엔진은 M2에서도 "한 문서 내용 스트림"만 — `doc_id`=`page_id`로 1:1. 트리 구조는 doc-service REST가 단독 소유.
