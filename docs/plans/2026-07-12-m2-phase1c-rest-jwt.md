...
- [x] PR② 브랜치 분기(①머지 후) — 가드/도메인 행위 (TDD, `0b4c5f4`+`282d4cd`+`ee31b72`)
- [x] PR② Workspace REST (TDD, `ca48827`)
- [x] PR② Page tree REST + move 사이클 검사 (TDD, `609997a`)
- [x] PR② Page sharing REST (TDD, `1b9382e`)
- [x] PR② 전체 테스트 green(102건) + 크래프트 게이트 실행(java-expert+code-reviewer 병렬, 2026-07-17) — **findings 미반영, 아래 §PR② 게이트 findings 참조**
- [x] PR② push(사용자 승인, 2026-07-17) — `origin/feature/m2-doc-service-rest-pages` (`0b4c5f4`~`1b9382e`)
- [x] PR 오픈 + 머지(사용자 승인, 2026-07-17) — [backend PR #9](https://github.com/ressKim-io/weDocs-backend/pull/9) squash `290bf69`. CI 3종(gitleaks/dependency-submission/dependency-review) + **머지 후 main push 스캔 green**(allowlist SHA 비의존 검증). ⚠️ 게이트 findings는 **미반영 머지**(사용자 지시) → 후속 이월
- [x] 마감: plan done + M2 세션 plan 재개지점 갱신 + CLAUDE.md 갱신. dev-log 생략(신규 함정 = colima env뿐, 메모리+plan 주의에 기록)
...
## PR② 게이트 findings (2026-07-17 실행, 미반영 — 머지 전 처리)

> java-expert(바이트코드 검증 포함)+code-reviewer 병렬. CRITICAL 0. 사용자 지시로 반영 전 push 먼저 실행.

**HIGH (머지 전 반영 권장)**
1. **아카이브 "도달성 숨김" 주석 ↔ 쿼리 불일치** — `list()`는 각 행의 `archived`만 필터: 아카이브 부모의 비아카이브 자손이 목록에 고아로 남음(클라 트리 조립 규약 미확정). 해소: 캐스케이드 아카이브 or 재귀 CTE or 주석 정정+클라 규약 명시. + 자손 보유 페이지 아카이브 테스트 추가. (`PageTreeService.list`/`Page.archive`)
2. **락 이전 1차 캐시 stale(추정, 미실증)** — `move()`의 `requireEdit`가 락 획득 **전** 조상 체인을 L1 캐시에 적재 → 락 후 `assertNoCycle`의 `findById`가 캐시 히트로 stale parentId를 읽을 수 있음(겹치는 조상 체인 + 동시 이동 조합에서 사이클 커밋 가능성). 해소 후보: 락을 최우선 획득(페이지 최소 로드→락→`em.clear()`→검증) or 락 후 refresh. + 실제 2-tx 레이스 통합 테스트.

**MEDIUM** ③ `PageSharingService.requireSharableBy` = `WorkspaceAccessGuard.requireOwner` 로직 중복(주입 재사용으로 통합) ④ 통합테스트 3개의 `createWorkspace/createPage` 헬퍼 byte-동일 중복 → `RestTestSupport`로 ⑤ `rename_byEditor` DisplayName이 D-3 검증 주장하나 실제 액터는 owner — MEMBER 액터 REST 종단 테스트 0건 ⑥ "비멤버 공유 editor의 자식 생성 허용"(주석 명시) 테스트 부재 ⑦ (java-expert) PESSIMISTIC_WRITE 락 대기 무제한 + HikariCP 풀 기본 10 — VT 대량 동시 move 시 풀 고갈([A] M3 부하검증 추적 수준, lock timeout 힌트+풀 명시 설정 권장)

**LOW** canRead()=allowed() 중복(위임으로) · MAX_ANCESTOR_DEPTH 2곳 독립 선언 · listMine 무상한 주석에 SDD §15 링크 부재 · share 404 본문 미검증 · 무토큰 401 엔드포인트별 미검증 · enum/UUID 바인딩 400 테스트 부재 · 락 직렬화 실증 테스트 부재(HIGH-2와 연동)

**PASS 확인**: deleteById 멱등·save=merge UPSERT(바이트코드 검증) · 파생쿼리 5종 · Security 기본 거부 상속 · @Valid/@NotNull(Integer) · VT pinning 없음 · 크래프트 6종 [B] 나머지 전부.

## 재개 지점 (Resume)

> **이 plan은 완료(done)** — PR①([#7](https://github.com/ressKim-io/weDocs-backend/pull/7) `a7e3a27`)·PR②([#9](https://github.com/ressKim-io/weDocs-backend/pull/9) `290bf69`) 모두 머지, 전체 102건 green, 4레포 스캔 green.
> **이월된 잔여물**: **§PR② 게이트 findings**(위 섹션) — HIGH 2건(아카이브 도달성 불일치 · move 락 이전 L1 캐시 stale 추정)은 머지 전 미반영(사용자 지시). 후속 소PR로 반영 권장 — 진입점은 [M2 세션 plan](2026-06-30-m2-persistence-session.md) §재개지점.
> **주의**: 서비스 레포는 건별 승인(push·PR). 설계 노트: 교차 워크스페이스 parent = 생성·이동 모두 409 통일(requireEdit 통과자는 부모 존재를 이미 앎 — 비노출 불필요). 루트 이동 = 멤버십 요구(공유만 받은 비멤버 editor 차단, 루트 생성과 동일 규칙). 아카이브 = 단일 행 flag — 자손은 도달성으로 숨김(복원 시 통째 복귀). 로컬 테스트 환경: colima — `DOCKER_HOST=unix://$HOME/.colima/default/docker.sock TESTCONTAINERS_DOCKER_SOCKET_OVERRIDE=/var/run/docker.sock` 필요. **Boot 4.x 함정 3건**: ① `@WebMvcTest` = `spring-boot-starter-webmvc-test` 별도 스타터 ② Jackson 3 = `tools.jackson`, `asString()` ③ TC 2.x `postgresql.PostgreSQLContainer` 비제네릭. test application.yml은 main 완전 대체. `.gitleaksignore` 커밋 핀은 squash에서 무효 — 오탐 억제는 경로/룰 기반으로.
