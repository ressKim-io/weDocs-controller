...
> **다음(새 세션 진입점)**: **Phase 2 인증/인가**(backend `doc-service`+`ws-gateway` 레포 — branch+PR+건별 승인, [ADR-0014](../adr/0014-auth-authz-boundary.md)) — ① gateway WS 핸드셰이크 JWT 검증(`Sec-WebSocket-Protocol` 서브프로토콜) ② connect 시 `CheckPermission(user_id, page_id)`(1b `resolve()` 소비) ③ viewer write-block = gateway 1차 update drop + 엔진 방어(D-5) ④ 인가 실패 close code. 발급측(JWT/JWKS)은 1c①(#7)에서 완료 — 이번은 **검증측**. 이후 Phase 3(엔진 save·`build_client` flip)→4(복원)→5(outbox)→6(E2E).
