...
> **마지막 완료**: **Phase 1c PR① 인증 기반 — backend PR #7 머지 완료**(squash `a7e3a27`, 2026-07-13). RS256 JWT 발급(`JwtKeys`/`JwtTokenService`, kid=RFC 7638 thumbprint, ADR-0017) + JWKS 공개 엔드포인트 + Security 배선(stateless·기본 거부·`@CurrentUserId`) + signup/login REST + 도메인 예외 계층·ProblemDetail(RFC 9457). 크래프트 게이트 2-리뷰 반영, 53건 green. 상세 = [2026-07-12-m2-phase1c-rest-jwt.md](2026-07-12-m2-phase1c-rest-jwt.md)(in-progress — PR② 잔여). Boot 4.x 함정 3건 = [dev-log](../dev-logs/2026-07-13-m2-doc-service-1c-boot4-traps.md). 1b(PR #6, `1d7ce7b`)·1a(PR #3, `54a8b48`)는 그 이전 완료.
> **다음(새 세션 진입점)**: **① backend main security-scan 적신호 해소**(gitleaks 오탐 2건 — `.gitleaksignore` 커밋 핀이 squash 머지로 무효화. controller 자신의 주간 schedule red 2건도 동반. 상세 = [1c plan §머지 후 발견](2026-07-12-m2-phase1c-rest-jwt.md)) → **② Phase 1c PR② 리소스 REST** — workspace·멤버 초대·페이지 CRUD(트리 이동 사이클 검사)·공유 API. `weDocs-backend` 신규 브랜치 `feature/m2-doc-service-rest-pages`(동일 branch+PR+승인 패턴). 설계는 1c plan §PR②에 확정 — 신규 결정 불필요, 구현만.
> **주의**: 서비스 레포(backend/crdt-engine/frontend)는 전부 건별 승인(브랜치·PR·push). controller만 main 직접. 엔진 `build_client(false)→true` flip은 Phase 3. **proto 태그 push·다운스트림 `ref` bump = 승인 게이트**(CI 정합 시). 게이트 트랙(T3 done·T4) = [plan-audit](2026-06-30-plan-audit-improvements.md). 1a 빌드 함정 = [dev-log](../dev-logs/2026-06-30-m2-doc-service-1a-version-traps.md)(Spring Boot 4.x=스타터 필수·TC 2.x=좌표) — 1c도 REST/Security 스타터 추가 시 동일 원칙 적용.
