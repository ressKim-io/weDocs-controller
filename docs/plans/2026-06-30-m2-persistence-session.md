...
> **다음(새 세션 진입점)**: **Phase 2 인증/인가 진행 중**(backend `ws-gateway`+`doc-service` — branch+PR+건별 승인, [ADR-0014](../adr/0014-auth-authz-boundary.md)/[0021](../adr/0021-ws-handshake-auth-failure-observability.md)). ✅ **2a-1 gateway WS 핸드셰이크 JWT 검증 완료**([backend PR #16](https://github.com/ressKim-io/weDocs-backend/pull/16), `19d7716`, **push·PR 완료·머지 대기**, 69 green, [dev-log](../dev-logs/2026-07-19-m2-gateway-authn-observability.md)). **→ 다음 = 2a-2**(PR #16 머지 후 새 브랜치): connect 시 `CheckPermission(user_id, page_id)`(1b `resolve()` 소비) + authz **403** + viewer write-drop 1차(D-5) + role 메타 엔진 전달 + 관측(`checkpermission_duration`·`authz_backend_error_total`). 이후 2b(engine role 방어)→2c(frontend 토큰). 발급측(JWT/JWKS)은 1c①(#7). **상세 plan·SSOT = [2026-07-19-m2-phase2-auth-authz.md](2026-07-19-m2-phase2-auth-authz.md) §재개 지점**. 이후 Phase 3(엔진 save·`build_client` flip)→4(복원)→5(outbox)→6(E2E).
