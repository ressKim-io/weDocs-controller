...
> **마지막 완료**: **Phase 1c 전체 완료**(2026-07-17) — PR② 리소스 REST [backend PR #9](https://github.com/ressKim-io/weDocs-backend/pull/9) 머지(squash `290bf69`): workspace 생성/목록/멤버 초대(owner 전용) + 페이지 트리 CRUD/move(워크스페이스 행 `PESSIMISTIC_WRITE` 직렬화 + 사이클 fail-closed 64) + 아카이브(가역) + 공유 PUT/DELETE(멱등, owner 전용) + 인가 가드 2종(no-read=404 비노출/403). 전체 102건 green, 머지 후 main 스캔 green. PR①(#7 `a7e3a27`)·1b(#6 `1d7ce7b`)·1a(#3 `54a8b48`)는 그 이전. 1c plan = [2026-07-12-m2-phase1c-rest-jwt.md](2026-07-12-m2-phase1c-rest-jwt.md)(**done**).
> **다음(새 세션 진입점)**: ① **1c PR② 게이트 findings 반영 소PR**(backend 새 브랜치) — HIGH 2건(아카이브 "도달성 숨김" 주석↔쿼리 불일치 · move 락 이전 L1 캐시 stale 추정) + MEDIUM/LOW, 목록 = [1c plan §PR② 게이트 findings](2026-07-12-m2-phase1c-rest-jwt.md) → ② **secure-coding retrofit**([plan](2026-07-03-secure-coding-retrofit.md), Phase 2 분기 전 P0=3-레포 DoS 체인) → ③ **Phase 2 인증/인가**(gateway JWT 검증·WS 핸드셰이크·viewer write-drop, ADR-0014). ①과 ②는 같은 backend 품질 트랙이라 한 세션에 묶어도 됨.
