...
> **다음(새 세션 진입점)**: **Phase 2 인증/인가 진행 중**(backend `ws-gateway`+`doc-service` — branch+PR+건별 승인, [ADR-0014](../adr/0014-auth-authz-boundary.md)/[0021](../adr/0021-ws-handshake-auth-failure-observability.md)). ✅ **2a-1 인증**([backend PR #16](https://github.com/ressKim-io/weDocs-backend/pull/16) squash `583b065`, 69 green, [dev-log](../dev-logs/2026-07-19-m2-gateway-authn-observability.md)) · ✅ **2a-2 인가 + viewer write-drop 1차**([backend PR #17](https://github.com/ressKim-io/weDocs-backend/pull/17) squash `4cb750d`, main 100 green, VT pinning 종결 [dev-log](../dev-logs/2026-07-20-vt-pinning-grpc-blocking-stub.md)) **둘 다 머지**. · ✅ **2b crdt-engine role 강제**([crdt-engine PR #11](https://github.com/ressKim-io/weDocs-crdt-engine/pull/11) squash `4d9c39e`, 게이트 [B] 0, 30 green) **셋 다 머지**. **→ 다음 = 2c frontend 토큰**(+ 데모 room UUID화 이월)이 Phase 2의 마지막. ⚠️ 2c 착수 전 **CI 갭 사이드트랙 권고** — 서비스 3레포에 빌드·테스트 CI 부재(security-scan만). ⚠️ 2b의 방어 범위 = **게이트웨이 회귀·계약 위반**이지 "엔진 직접 gRPC 우회" 차단이 아니다(`role`은 클라 통제 메타 — 악의적 직접 호출자는 `editor` 자칭 통과, 차단은 M5 mTLS/NetworkPolicy). 발급측(JWT/JWKS)은 1c①(#7). **상세 plan·SSOT = [2026-07-19-m2-phase2-auth-authz.md](2026-07-19-m2-phase2-auth-authz.md) §재개 지점**. 이후 Phase 3(엔진 save·`build_client` flip)→4(복원)→5(outbox)→6(E2E).
