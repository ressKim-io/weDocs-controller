...
> **마지막 완료**: **Phase 1b gRPC 서버 — backend PR #6 머지 완료**(`1d7ce7b`, 2026-07-04). `DocService` 4 RPC(`CheckPermission`/`SaveSnapshot`/`LoadSnapshot`/`GetDocMeta`) + `WorkspaceMember`/`PagePermission` 복합키 엔티티(이 코드베이스 최초 `@EmbeddedId`) + PRD §4.2 권한 해석(조상 탐색 캡 fail-closed) + gRPC 서버 배선(SmartLifecycle+VT executor). 2라운드 리뷰(크래프트 게이트 java-expert+code-reviewer, max-effort 10-앵글) 전부 반영, 32개 테스트 green. 상세 = [2026-07-04-m2-phase1b-docservice-grpc.md](2026-07-04-m2-phase1b-docservice-grpc.md)(done).
> **다음(새 세션 진입점)**: **Phase 1c REST + 인증 발급** — 페이지 CRUD(트리 이동 사이클 검사)·workspace·멤버 초대 REST + JWT 발급(로그인). `weDocs-backend` 신규 브랜치(1b와 동일 branch+PR+승인 패턴). 검증 발급=1c, gateway 검증=Phase 2. proto/ADR은 이미 확정(0012~0015) — 신규 결정 불필요, 구현만.
