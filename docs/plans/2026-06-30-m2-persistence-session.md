...
> **다음(새 세션 진입점)**: **Phase 1c PR② 리소스 REST** — workspace·멤버 초대·페이지 CRUD(트리 이동 사이클 검사)·공유 API. `weDocs-backend` 신규 브랜치 `feature/m2-doc-service-rest-pages`(동일 branch+PR+승인 패턴). 설계는 [1c plan §PR②](2026-07-12-m2-phase1c-rest-jwt.md)에 확정 — 신규 결정 불필요, 구현만. (선행이던 security-scan 적신호는 2026-07-17 해소 — [plan](2026-07-17-gitleaks-false-positive-baseline.md) done, 4레포 전 트리거 green.)
