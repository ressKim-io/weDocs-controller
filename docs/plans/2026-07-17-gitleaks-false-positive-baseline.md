...
- [ ] **PR #8 머지 → main CI green 확인** ← 이 작업의 최종 증명(squash 후에도 안 깨지는지)
- [ ] 마감: plan done + 1c plan/CLAUDE.md 재개지점 갱신(다음=PR②)
...
> **마지막 완료**: **controller 완료**(`9d62d65` + workflow_dispatch — 실 CI 전체 히스토리 스캔 green 확인, 2주 묵은 red 해소) + **backend [PR #8](https://github.com/ressKim-io/weDocs-backend/pull/8) 오픈·PR CI 3종 green** + 표준/dev-log 반영 완료.
> **다음**: **PR #8 머지(사용자 승인) → main CI green 확인**(= squash 후에도 안 깨진다는 최종 증명) → plan done → **M2 Phase 1c PR② 착수**([1c plan §PR②](2026-07-12-m2-phase1c-rest-jwt.md)).
