...
- [ ] controller: 이 plan 커밋(planned)
- [ ] controller `.gitleaks.toml` 추가 → 로컬 8.24.3 재현/대조군 green → 커밋·push → **실 CI green 확인**(push + schedule 수동 트리거)
- [ ] backend: 브랜치 `chore/gitleaks-fp-baseline` 분기 → `.gitleaks.toml` 추가 + `.gitleaksignore` 삭제 → 로컬 대조군 green
- [ ] backend: 사용자 승인 후 push·PR 오픈 → **머지 후 main CI green 확인**(squash 후에도 안 깨지는지가 이 작업의 핵심 증명)
- [ ] 마감: plan done + dev-log(fingerprint 트랩 = 재발 방지 SOP) + 1c plan/CLAUDE.md 재개지점 갱신(다음=PR②)
...
> **마지막 완료**: 진단·검증 완료(오탐 4건 판정, 근본 원인=squash SHA 재작성, 후보 config 2종 실측 대조군 통과). plan 커밋 = 다음.
> **다음**: controller `.gitleaks.toml` 적용 → backend 브랜치(승인 후 push/PR).
> **주의**: `targetRules` 금지(8.25+ 전용, CI는 8.24.3). 경로 blanket 금지 — 반드시 `condition="AND"`로 값/모양까지 좁힌다. backend는 건별 승인.
