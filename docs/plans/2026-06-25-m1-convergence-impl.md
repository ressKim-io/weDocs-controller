...
- [x] 4.2 **gateway(Java) — OTel javaagent 부착** ✅ **PR #2 머지 완료**(merge commit 30e0aca, 2026-06-30, backend main 동기화·브랜치 삭제): https://github.com/ressKim-io/weDocs-backend/pull/2 (2커밋: cf4be94 javaagent 설정, c8374d5 OTLP protocol=grpc 수정). 앱 코드 0. `make run-otel`+`OTEL_METRICS_EXPORTER=none OTEL_LOGS_EXPORTER=none`(M1 thin=trace만). 아래는 구현 명세(backend 레포 = 앱 코드 0):
...
- [x] 4.3 **live 검증(실측) ✅ 완료**(2026-06-30 Linux, docker 29.6+Jaeger v2 2.19.0):
  - **★단일 폴리글랏 trace 실측 성공**(가드레일 4): `traceID=eb852a8df8473e52ee0b732d69a92552` — `[ws-gateway] crdt.CrdtEngine/Sync`(ROOT) → `[wedocs-crdt-engine] crdt.sync`(parent=gateway span). **Java span→Rust span 같은 trace, 부모-자식**. Jaeger API(`/api/traces?service=ws-gateway`)로 두 서비스 동일 traceID 확인.
  - **★OTLP 프로토콜 함정 발견·수정**(config-contract-audit 사례): javaagent 2.x **기본 OTLP 프로토콜=http/protobuf**로 바뀌어, endpoint=4317(gRPC 포트)에 HTTP로 보내 `unexpected end of stream` export 실패. → `OTEL_EXPORTER_OTLP_PROTOCOL=grpc`로 engine과 통일(c8374d5). 수정 후 export 에러 0.
  - **endpoint 검증(M1R-09 해소)**: engine `with_tonic().build()`가 `OTEL_EXPORTER_OTLP_ENDPOINT` 자동 읽음 — 로그 `exporter=otlp/grpc → http://localhost:4317` + engine span 도착으로 실증. `.with_endpoint()` 불요(코드 무변경).
  - **회귀(M1R-05)** ✅: javaagent 부착 gateway로 `npm run test:e2e` **green**(수렴 무손상) + Jaeger 단일 trace 둘 다 충족 = **Phase 4 done 게이트 통과**.
  - 기동: `docker compose -f infra/local/docker-compose.jaeger.yml up -d` → engine `cargo run`(OTLP/4317) → gateway `make run-otel` → e2e. (`make proto-sync` 선행, `cargo` PATH 주의.)
...
> **마지막 완료**: **Phase 4 OTel 폴리글랏 trace — 전부 완료**(2026-06-30). 4.1 engine(f2cdea2)·**4.2 gateway javaagent PR #2 머지**(30e0aca)·**4.3 live 검증 ✅**. **★단일 trace 실측**(traceID `eb852a8d…`: `ws-gateway crdt.CrdtEngine/Sync` → `wedocs-crdt-engine crdt.sync` 부모-자식, 가드레일 4) + 회귀 E2E green. Phase 1~3 수렴(e8f0c83·e0e8277·fbd25fe). **Phase 4 done 게이트 통과 = 단일 trace + 수렴 둘 다.**
> **다음 작업 = Phase 5 마감**(controller, main 직접): ①Phase 4 engine+gateway OTel 후향 dev-log(PDD-01, OTLP protocol=grpc 함정·단일 trace 실측 포함) ②M1 수렴 종합 dev-log/retrospective(M1R-07) ③ADR **0011** 엔진 sync/fan-out 브리지(대안 3축) ④SDD §15 미해결 5건 소유 마일스톤 갱신(DOC-01) ⑤이 plan `status: done`. → 그다음 **M2([T3 readiness](2026-06-30-plan-audit-improvements.md), M2F-02 blocker ADR 먼저)**. **검증된 한계**: stream-open 1회 → "한 WS 세션이 Java span→Rust span"(per-edit=M5).
> **★검증된 핵심(변경 금지)**: gateway는 `OTEL_EXPORTER_OTLP_PROTOCOL=grpc` 필수(javaagent 2.x 기본 http/protobuf가 gRPC 4317서 실패). engine `telemetry.rs`는 `with_tonic().build()`가 endpoint env 자동 읽음 → 코드 무변경(`.with_endpoint()` 금지=signal-specific override 덮어씀). 4.3 재현: `make proto-sync` 선행 + `cargo`/`buf` PATH(`~/.cargo/bin`·`~/.local/bin`).
