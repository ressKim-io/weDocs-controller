...
- [x] branch push + **PR #2 open**(2026-06-29, 사용자 승인): https://github.com/ressKim-io/weDocs-crdt-engine/pull/2 (커밋 3f02722) → 머지 대기. 4.2 게이트웨이(javaagent 기동 설정)는 별도 PR
...
> **다음 작업**: **Phase 4.1 engine PR #2 머지 대기**(https://github.com/ressKim-io/weDocs-crdt-engine/pull/2, 커밋 3f02722). 머지 후 → **4.2 게이트웨이**(OTel **javaagent** 기동 설정 = `-javaagent`+`OTEL_SERVICE_NAME=ws-gateway`/`OTEL_EXPORTER_OTLP_ENDPOINT`/`OTEL_TRACES_EXPORTER=otlp`, **앱 코드·build.gradle 변경 0**, backend 별도 PR+승인) → **4.3 live 검증은 Linux**(docker+Jaeger, `OTEL_BSP_SCHEDULE_DELAY=1000`로 5s delay 단축, gateway↔engine trace_id 일치+단일 trace UI) → Phase 5 마감 → **M2(영속화·세션·권한, doc-service 신설)**. **검증된 한계**: stream-open 1회 → "한 WS 세션이 Java span→Rust span"(per-edit=M5).
