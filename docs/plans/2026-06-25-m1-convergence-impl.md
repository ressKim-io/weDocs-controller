...
- [~] 4.2 **gateway(Java) — OTel javaagent 부착** — 구현+검증 완료, **PR #2 open(머지 대기)**: https://github.com/ressKim-io/weDocs-backend/pull/2 (커밋 cf4be94, branch `feature/m1-otel-gateway-javaagent`). 검증(2026-06-30 Linux): `make -n` 체인·agent v2.29.0 부착(VersionLogger)·bootJar BUILD SUCCESSFUL(23s)·**gateway 부팅 green**(Started 1.16s·Tomcat 8080·Java 25 VT). 단일 trace 실측=4.3. 스모크 발견 반영: `OTEL_METRICS_EXPORTER=none OTEL_LOGS_EXPORTER=none`(M1 thin=trace만). 아래는 구현 명세(backend 레포 = 앱 코드 0):
...
> **다음 작업(2026-06-30 진행 중)**: **Phase 4.2 = 구현+검증 완료, backend PR #2 open**(https://github.com/ressKim-io/weDocs-backend/pull/2, 커밋 cf4be94). gateway 부팅 green(agent v2.29.0·Java25 VT). **머지는 사용자 승인 대기.** → **4.3 live 검증**(이 Linux서 실행 가능, docker 29.6): `docker compose -f infra/local/docker-compose.jaeger.yml up -d` → engine `cargo run` → gateway `make run-otel`(feature 브랜치 working tree) → WS 1회 접속 → Jaeger UI 단일 trace(ws-gateway→wedocs-crdt-engine) + 회귀 E2E green(둘 다=done 게이트). 4.3은 4.2 머지 전에도 feature 브랜치로 실행 가능. → Phase 5 마감(dev-log 4.1 후향·M1 회고·ADR 0011·SDD §15 정리) → **M2([T3 readiness](2026-06-30-plan-audit-improvements.md))**. **검증된 한계**: stream-open 1회 → "한 WS 세션이 Java span→Rust span"(per-edit=M5).
