...
- [ ] branch push + PR (engine, 건별 승인) → 4.2 게이트웨이(javaagent 기동 설정)는 별도
...
> **다음 작업**: **Phase 4.1 engine PR push 승인 대기**(커밋 3f02722, 브랜치 `feature/m1-otel-trace`). §4.0 검증·구현·로컬검증(build/clippy/fmt/test 8 pass)·otel-expert cross-check(§4.4, fix 반영) 완료 → **engine push+PR만 남음(건별 승인)**. 추출=수동(`MetadataExtractor`→`set_parent`→`.instrument`), 익스포터=OTEL_TRACES_EXPORTER로 otlp/stdout, shutdown=spawn_blocking. **검증된 한계**: stream-open 1회 → "한 WS 세션이 Java span→Rust span 단일 trace"(per-edit=M5). 이후 = **4.2 게이트웨이**(OTel javaagent 기동 설정, 앱 코드 0) → **4.3 live 검증은 Linux**(docker+Jaeger, `OTEL_BSP_SCHEDULE_DELAY=1000`) → Phase 5 마감 → **M2(영속화·세션·권한, doc-service 신설)**.
