...
### 클라우드/파괴적 CLI
- 모든 destructive operation(삭제·교체·스케일·권한 변경)은 사전 승인
- **`--force` / `--quiet` 플래그를 AI가 임의로 덧붙이지 않는다** — 확인 프롬프트 생략·의존성 체크 우회
...
사용자가 "알아서 해", "맘껏 해", "빨리 진행해", "바로 해줘" 같은 **포괄적 위임**을 했더라도, 이 파일의 사전 승인 필수 작업은 **건별로 개별 승인**을 받는다.
